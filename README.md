# LeetCode Algorithms

A modular collection of LeetCode solutions written in C# (.NET 8), organized by topic and difficulty.

## Statistics

| Metric | Count |
|---|---:|
| Solution files | 427 |
| Unique LeetCode problems | 421 |
| Easy (unique) | 368 |
| Medium (unique) | 52 |
| Hard (unique) | 1 |
| Topic categories | 14 |
| Incomplete files | 36 |
| Needs-review files | 9 |

> Difficulty counts use official LeetCode metadata. Duplicate files count once in unique totals.

## Topics

| Topic | Files | Unique | Easy | Medium | Hard |
|---|---:|---:|---:|---:|---:|
| [Array](Algorithms/Array/README.md) | 129 | 129 | 94 | 35 | 0 |
| [Backtracking](Algorithms/Backtracking/README.md) | 1 | 1 | 0 | 1 | 0 |
| [BinarySearch](Algorithms/BinarySearch/README.md) | 3 | 3 | 3 | 0 | 0 |
| [BitManipulation](Algorithms/BitManipulation/README.md) | 1 | 1 | 1 | 0 | 0 |
| [DynamicProgramming](Algorithms/DynamicProgramming/README.md) | 5 | 5 | 5 | 0 | 0 |
| [Greedy](Algorithms/Greedy/README.md) | 16 | 16 | 15 | 1 | 0 |
| [HashTable](Algorithms/HashTable/README.md) | 51 | 51 | 50 | 0 | 1 |
| [LinkedList](Algorithms/LinkedList/README.md) | 6 | 6 | 4 | 2 | 0 |
| [Math](Algorithms/Math/README.md) | 93 | 93 | 91 | 2 | 0 |
| [Sorting](Algorithms/Sorting/README.md) | 1 | 1 | 1 | 0 | 0 |
| [Stack](Algorithms/Stack/README.md) | 4 | 4 | 4 | 0 | 0 |
| [String](Algorithms/String/README.md) | 106 | 106 | 96 | 10 | 0 |
| [Tree](Algorithms/Tree/README.md) | 4 | 4 | 3 | 1 | 0 |
| [TwoPointers](Algorithms/TwoPointers/README.md) | 7 | 7 | 7 | 0 | 0 |

## Repository audit

### Incomplete implementations

- [2 - Add Two Numbers](Algorithms/LinkedList/Medium/AddTwoNumbers.cs)
- [17 - Letter Combinations of a Phone Number](Algorithms/Backtracking/Medium/LetterCombinationsOfAPhoneNumber.cs)
- [21 - Merge Two Sorted Lists](Algorithms/LinkedList/Easy/MergeTwoSortedLists.cs)
- [78 - Subsets](Algorithms/Array/Medium/Subsets.cs)
- [108 - Convert Sorted Array to Binary Search Tree](Algorithms/Tree/Easy/ConvertSortedArrayToBinarySearchTree.cs)
- [139 - Word Break](Algorithms/Array/Medium/WordBreak.cs)
- [257 - Binary Tree Paths](Algorithms/Tree/Easy/BinaryTreePaths.cs)
- [383 - Ransom Note](Algorithms/String/Easy/RansomNote.cs)
- [495 - Teemo Attacking](Algorithms/Greedy/Easy/TeemoAttacking.cs)
- [500 - Keyboard Row](Algorithms/String/Easy/KeyboardRow.cs)
- [509 - Fibonacci Number](Algorithms/Math/Easy/FibonacciNumber.cs)
- [680 - Valid Palindrome II](Algorithms/String/Easy/ValidPalindromeII.cs)
- [706 - Design HashMap](Algorithms/HashTable/Easy/DesignHashMap.cs)
- [821 - Shortest Distance to a Character](Algorithms/String/Easy/ShortestDistanceToACharacter.cs)
- [922 - Sort Array By Parity II](Algorithms/Array/Easy/SortArrayByParityII.cs)
- [1002 - Find Common Characters](Algorithms/HashTable/Easy/FindCommonCharacters.cs)
- [1011 - Capacity To Ship Packages Within D Days](Algorithms/Array/Medium/CapacityToShipPackagesWithinDDays.cs)
- [1160 - Find Words That Can Be Formed by Characters](Algorithms/HashTable/Easy/FindWordsThatCanBeFormedByCharacters.cs)
- [1189 - Maximum Number of Balloons](Algorithms/String/Easy/MaximumNumberOfBalloons.cs)
- [1277 - Count Square Submatrices with All Ones](Algorithms/Array/Medium/CountSquareSubmatricesWithAllOnes.cs)
- [1304 - Find N Unique Integers Sum up to Zero](Algorithms/Math/Easy/FindNUniqueIntegersSumUpToZero.cs)
- [1347 - Minimum Number of Steps to Make Two Strings Anagram](Algorithms/String/Medium/MinimumNumberOfStepsToMakeTwoStringsAnagram.cs)
- [1408 - String Matching in an Array](Algorithms/String/Easy/StringMatchingInAnArray.cs)
- [1415 - The k-th Lexicographical String of All Happy Strings of Length n](Algorithms/String/Medium/TheKThLexicographicalStringOfAllHappyStringsOfLengthN.cs)
- [1732 - Find the Highest Altitude](Algorithms/Array/Easy/FindTheHighestAltitude.cs)
- [2027 - Minimum Moves to Convert String](Algorithms/Greedy/Easy/MinimumMovesToConvertString.cs)
- [2138 - Divide a String Into Groups of Size k](Algorithms/String/Easy/DivideAStringIntoGroupsOfSizeK.cs)
- [2269 - Find the K-Beauty of a Number](Algorithms/Math/Easy/FindTheKBeautyOfANumber.cs)
- [2273 - Find Resultant Array After Removing Anagrams](Algorithms/HashTable/Easy/FindResultantArrayAfterRemovingAnagrams.cs)
- [2375 - Construct Smallest Number From DI String](Algorithms/String/Medium/ConstructSmallestNumberFromDIString.cs)
- [2482 - Difference Between Ones and Zeros in Row and Column](Algorithms/Array/Medium/DifferenceBetweenOnesAndZerosInRowAndColumn.cs)
- [2506 - Count Pairs Of Similar Strings](Algorithms/HashTable/Easy/CountPairsOfSimilarStrings.cs)
- [2843 - Count Symmetric Integers](Algorithms/Math/Easy/CountSymmetricIntegers.cs)
- [3379 - Transformed Array](Algorithms/Array/Easy/TransformedArray.cs)
- [3446 - Sort Matrix by Diagonals](Algorithms/Array/Medium/SortMatrixByDiagonals.cs)
- [4000 - Largest Integer With Given Digit Sum](Algorithms/Greedy/Easy/LargestIntegerWithGivenDigitSum.cs)

### Needs review

- [28 - Find the Index of the First Occurrence in a String](Algorithms/String/Easy/FindTheIndexOfTheFirstOccurrenceInAString.cs)
- [55 - Jump Game](Algorithms/Greedy/Easy/JumpGame.cs)
- [953 - Verifying an Alien Dictionary](Algorithms/HashTable/Easy/VerifyingAnAlienDictionary.cs)
- [1991 - Find the Middle Index in Array](Algorithms/Array/Easy/FindTheMiddleIndexInArray.cs)
- [2515 - Shortest Distance to Target String in a Circular Array](Algorithms/String/Easy/ShortestDistancetoTargetStringInACircularArray.cs)
- [2706 - Buy Two Chocolates](Algorithms/Greedy/Easy/BuyTwoChocolates.cs)
- [3046 - Split the Array](Algorithms/Array/Easy/SplitTheArray.cs)
- [3178 - Find the Child Who Has the Ball After K Seconds](Algorithms/Math/Easy/FindTheChildWhoHasTheBallAfterKSeconds.cs)
- [3718 - Smallest Missing Multiple of K](Algorithms/HashTable/Easy/SmallestMissingMultipleOfK.cs)

### Duplicate problems

- **1160 - Find Words That Can Be Formed by Characters**
  - [Algorithms/Array/Easy/FindWordsThatCanBeFormedByCharacters.cs](Algorithms/Array/Easy/FindWordsThatCanBeFormedByCharacters.cs)
  - [Algorithms/HashTable/Easy/FindWordsThatCanBeFormedByCharacters.cs](Algorithms/HashTable/Easy/FindWordsThatCanBeFormedByCharacters.cs)
- **1207 - Unique Number of Occurrences**
  - [Algorithms/Array/Easy/UniqueNumberOfOccurrences.cs](Algorithms/Array/Easy/UniqueNumberOfOccurrences.cs)
  - [Algorithms/HashTable/Easy/UniqueNumberOfOccurrences.cs](Algorithms/HashTable/Easy/UniqueNumberOfOccurrences.cs)
- **1304 - Find N Unique Integers Sum up to Zero**
  - [Algorithms/Array/Easy/FindNUniqueIntegersSumUpToZero.cs](Algorithms/Array/Easy/FindNUniqueIntegersSumUpToZero.cs)
  - [Algorithms/Math/Easy/FindNUniqueIntegersSumUpToZero.cs](Algorithms/Math/Easy/FindNUniqueIntegersSumUpToZero.cs)
- **1331 - Rank Transform of an Array**
  - [Algorithms/Array/Easy/RankTransformOfAnArray.cs](Algorithms/Array/Easy/RankTransformOfAnArray.cs)
  - [Algorithms/HashTable/Easy/RankTransformOfAnArray.cs](Algorithms/HashTable/Easy/RankTransformOfAnArray.cs)
- **2269 - Find the K-Beauty of a Number**
  - [Algorithms/Math/Easy/FindTheKBeautyOfANumber.cs](Algorithms/Math/Easy/FindTheKBeautyOfANumber.cs)
  - [Algorithms/String/Easy/FindTheKBeautyOfANumber.cs](Algorithms/String/Easy/FindTheKBeautyOfANumber.cs)
- **2566 - Maximum Difference by Remapping a Digit**
  - [Algorithms/Greedy/Easy/MaximumDifferenceByRemappingADigit.cs](Algorithms/Greedy/Easy/MaximumDifferenceByRemappingADigit.cs)
  - [Algorithms/Math/Easy/MaximumDifferenceByRemappingADigit.cs](Algorithms/Math/Easy/MaximumDifferenceByRemappingADigit.cs)

### Difficulty-folder mismatches

- [55 - Jump Game](Algorithms/Greedy/Easy/JumpGame.cs): folder `Easy`, official **Medium**.

## Ordering convention

- README indexes use the LeetCode problem ID as the canonical order.
- Descriptive file names are retained for alphabetical IDE and repository search.
- Every solution begins with verified ID, title, topic, difficulty, and link metadata.

## Build and run

```bash
dotnet build LeetCode.sln
dotnet run --project Algorithms/Array/Array.csproj
```

Topic `Program.cs` files are demo runners; topic README files are the complete indexes.

## Solution header

```csharp
/**
 * @id         2481
 * @title      Minimum Cuts to Divide a Circle
 * @topic      Math
 * @difficulty Easy
 * @link       https://leetcode.com/problems/minimum-cuts-to-divide-a-circle/
 */
```
